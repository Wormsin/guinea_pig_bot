...
async def cmd_start(message: types.Message):
    await message.answer(
        ...
        "Чтобы узнать подробнее о тесте и оплате — введите команду /info.\n"
        "Если вы готовы пройти регистрацию: назначить время прохождения и получить ссылку — используйте /register."
    )
...
async def cmd_info(message: types.Message):
    await message.answer(
        ...
        "📋 Вам предстоит пройти онлайн-тест продолжительностью около 1 часа. Тест нужно проходить на **ПК или ноутбуке** — на телефонах он не работает корректно.\n\n"
        "🧠 В ходе теста будет использоваться **трекинг взгляда с помощью камеры** — в начале будет калибровка, чтобы всё работало корректно. Задания будут включать **поиск объектов, запоминание и другие простые когнитивные задачи**.\n\n"
        "❗Очень важно проходить тест **внимательно и без отвлечений**. Лучше выбрать **тихое место** и не слишком позднее время. Если вы отвлечётесь, тест может завершиться некорректно.\n\n"
        "😌 Не переживайте, если что-то не получилось или вы чувствовали себя «тупо» — это нормально! Мы **не оцениваем участников**, результаты **анонимны** и **не влияют на оплату**. Главное — пройти тест честно.\n\n"
        ...
        "💰 После подтверждения прохождения теста вы получите вознаграждение **в размере 700 рублей** на указанный номер телефона.\n\n"
        "Чтобы принять участие, используйте команду /register."
    )
